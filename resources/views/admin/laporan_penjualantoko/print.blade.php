        ...
        <table>
            ...
            <tbody>
                ...
                @foreach($setoranPenjualans as $setoran)
                    <tr>
                        <td>{{ \Carbon\Carbon::parse($setoran->tanggal_setoran)->translatedFormat('d F Y') }}</td>
                        ...
                        <td>{{ number_format($setoran->mesin_edc, 0, ',', '.') }}</td>
                        <td>{{ number_format($setoran->gobiz, 0, ',', '.') }}</td>
                        <td>{{ number_format($setoran->transfer, 0, ',', '.') }}</td>
                        <td>{{ number_format($setoran->qris, 0, ',', '.') }}</td>
                        <td>{{ number_format($setoran->total_setoran, 0, ',', '.') }}</td>
                        <td>{{ number_format($setoran->nominal_setoran, 0, ',', '.') }}</td>
                        <td>{{ number_format($setoran->plus_minus, 0, ',', '.') }}</td>
                    </tr>
            ...
            <tfoot>
                <tr style="font-weight: bold;">
                    ...
                    <td>{{ number_format($totalPenjualanKotor, 0, ',', '.') }}</td>
                    <td>{{ number_format($totalDiskonPenjualan, 0, ',', '.') }}</td>
                    <td>{{ number_format($totalPenjualanBersih, 0, ',', '.') }}</td>
                    <td>{{ number_format($totalDepositKeluar, 0, ',', '.') }}</td>
                    <td>{{ number_format($totalDepositMasuk, 0, ',', '.') }}</td>
                    <td>{{ number_format($totalTotalPenjualan, 0, ',', '.') }}</td>
                    <td>{{ number_format($totalMesinEDC, 0, ',', '.') }}</td>
                    <td>{{ number_format($totalGoBiz, 0, ',', '.') }}</td>
                    <td>{{ number_format($totalTransfer, 0, ',', '.') }}</td>
                    <td>{{ number_format($totalQRIS, 0, ',', '.') }}</td>
                    <td>{{ number_format($totalTotalSetoran, 0, ',', '.') }}</td>
                    <td>{{ number_format($totalNominalSetoran, 0, ',', '.') }}</td>
                    <td>{{ number_format($totalPlusMinus, 0, ',', '.') }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
