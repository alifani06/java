                            ...
                            <tbody>
                                @foreach ($produks as $item)
                                    @php
                                        ...
                                        $stok_tokotegal = $item->stok_tokotegal ? $item->stok_tokotegal->jumlah : 0;
                                    @endphp
                                    <tr>
                                        <td class="text-center">{{ $loop->iteration }}</td>
                                        ...
                                        <td>
                                            <span class="member_harga_tgl">{{ $tokotegal ? $tokotegal->member_harga_tgl : 'N/A' }}</span>
                                        </td>
                                        <td>
                                            <span class="member_diskon_tgl">{{ $tokotegal ? $tokotegal->member_diskon_tgl : 'N/A' }}</span>
                                        </td>
                                        <td>
                                            <span class="non_harga_tgl">{{ $tokotegal ? $tokotegal->non_harga_tgl : 'N/A' }}</span>
                                        </td>
                                        <td>
                                            <span class="non_diskon_tgl">{{ $tokotegal ? $tokotegal->non_diskon_tgl : 'N/A' }}</span>
                                        </td>
                                        <td class="text-center">{{ $stok_tokotegal }}</td>
...
<script>
    $(document).ready(function() {
        ...
        function updatePrices(tipePelanggan) {
            $('#datatables5 tbody tr').each(function() {
                var hargaMember = parseFloat($(this).find('.member_harga_tgl').text()) || 0;
                var diskonMember = parseFloat($(this).find('.member_diskon_tgl').text()) || 0;
                var hargaNonMember = parseFloat($(this).find('.non_harga_tgl').text()) || 0;
                var diskonNonMember = parseFloat($(this).find('.non_diskon_tgl').text()) || 0;

                if (tipePelanggan === 'member') {
                    // Update harga dan diskon member
                    $(this).find('.member_harga_tgl').text(hargaMember); 
                    $(this).find('.member_diskon_tgl').text(diskonMember); 
                } else if (tipePelanggan === 'nonmember') {
                    // Update harga dan diskon non-member
                    $(this).find('.non_harga_tgl').text(hargaNonMember); // Harga non-member
                    $(this).find('.non_diskon_tgl').text(diskonNonMember); // Diskon non-member
                }
            });
        }
    ...
    $('#tabel-pembelian-body').on('keydown', '.jumlah', function(event) {
        ...
        var harga, diskon;
        if (tipePelanggan === 'member') {
            harga = parseFloat(row.find('.member_harga_tgl').text()) || 0;
            diskon = parseFloat(row.find('.member_diskon_tgl').text()) || 0;
        } else if (tipePelanggan === 'nonmember') {
            harga = parseFloat(row.find('.non_harga_tgl').text()) || 0;
            diskon = parseFloat(row.find('.non_diskon_tgl').text()) || 0;
        }
