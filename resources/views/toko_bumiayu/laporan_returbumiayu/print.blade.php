...
<body>
    ...
    <div class="header">
        ...
        <h1 class="title2">LAPORAN BARANG RETUR</h1>
    ...
    <table>
        ...
        <tbody>
            @php $no = 1; $lastKlasifikasiId = null; @endphp
            @foreach($stokBarangJadi as $returGroup)
                @foreach($returGroup as $retur)
                    @if ($lastKlasifikasiId !== $retur->produk->klasifikasi_id)
                        @if ($lastKlasifikasiId !== null) 
                            <tr>
                                <td colspan="7" style="border: 2px solid black; height: 1px;"></td>
                            </tr>
                        @endif
                        @php $lastKlasifikasiId = $retur->produk->klasifikasi_id; @endphp
                    @endif
                    <tr>
