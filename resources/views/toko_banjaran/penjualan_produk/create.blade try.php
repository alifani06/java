...
{{-- <script>
  $(document).ready(function() {
    ...
    function calculateFee() {
        var metodeId = $('#nama_metode').val(); // Ambil nilai metode pembayaran yang dipilih
        var subTotal = parseFloat($('#sub_total').val().replace(/[^\d]/g, '')) || 0; // Ambil subtotal

        var feePercentage = 0; // Inisialisasi fee
        if (metodeId && metodeId !== "tunai") { // Tambahkan pengecekan untuk 'tunai'
            feePercentage = parseFloat($('#nama_metode option:selected').data('fee')) || 0; // Ambil fee dari data attribute
            
            // Hitung total fee dan tambahkan ke sub total
            var totalFee = (subTotal * feePercentage) / 100;
            var totalWithFee = subTotal + totalFee;
            
            // Format nilai fee dan subtotal dengan fee
            ...
            // Update nilai total fee dan subtotal di form
            $('#total_fee').val(formattedTotalFee); // Set total fee ke input
            $('#sub_total').val(formattedTotalWithFee); // Update subtotal dengan fee

            // Update input 'Uang Bayar' dengan total dengan fee
            $('#bayar').val(formattedTotalWithFee); // Set nilai uang bayar sama dengan subtotal yang telah ditambah fee
        } else {
            $('#total_fee').val('Rp0');
            // Jika 'Tunai' dipilih, jangan ubah subtotal
            if (metodeId !== "tunai") {
                $('#sub_total').val('Rp0'); // Reset subtotal tanpa tambahan fee jika metode kosong
            }
            $('#bayar').val(''); // Kosongkan input bayar jika tidak ada metode yang dipilih
        }
    }

    // Event listener untuk dropdown metode pembayaran
    $('#nama_metode').change(function() {
        var metodeId = $(this).val(); // Ambil metode yang dipilih

        // Tampilkan atau sembunyikan form berdasarkan metode pembayaran
        if (metodeId) {
            $('#payment-fields').show();

            // Cek apakah metode yang dipilih adalah "Tunai"
            if (metodeId === "tunai") {
                // Sembunyikan elemen Fee, Total Fee, dan Keterangan
                $('#fee').closest('.input-group').hide(); // Sembunyikan Fee
                $('#total_fee').closest('.col-md-4').hide(); // Sembunyikan Total Fee
                $('#keterangan').closest('.col-md-6').hide(); // Sembunyikan Keterangan
                
                // Mengembalikan subtotal ke nilai asli
                $('#sub_total').val($('#sub_totalasli').val()); 
            } else {
                // Tampilkan kembali elemen jika metode bukan 'Tunai'
                $('#fee').closest('.input-group').show(); // Tampilkan Fee
                $('#total_fee').closest('.col-md-4').show(); // Tampilkan Total Fee
                $('#keterangan').closest('.col-md-6').show(); // Tampilkan Keterangan
                
                calculateTotal(); // Panggil fungsi untuk menghitung subtotal dari item yang dibeli
                $('#total_fee').val('Rp0'); // Reset total fee
                $('#keterangan').val(''); // Kosongkan keterangan
            }

            calculateFee(); // Panggil fungsi untuk menghitung fee
            calculateKembali(); // Panggil fungsi untuk menghitung kembalian setelah update subtotal
        } else {
            $('#payment-fields').hide();
            $('#total_fee').val('Rp0'); // Reset total fee
            $('#sub_total').val('Rp0'); // Reset subtotal
            $('#keterangan').val(''); // Kosongkan keterangan
        }
    });
    ...
    function calculateTotal() {
        ...
        $('#sub_total').val(formattedSubtotal); // Tampilkan subtotal final (dengan diskon)
        $('#sub_totalasli').val(formattedSubtotalAsli); // Simpan subtotal asli dengan format
    ...
    // Event listener pada form input bayar agar menghitung ulang kembalian
    $('#bayar').on('blur', function() {
        formatAndUpdateKembali(); 
    });
