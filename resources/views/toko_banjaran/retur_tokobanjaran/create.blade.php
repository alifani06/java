...
document.addEventListener('DOMContentLoaded', function() {
 ...
 <script>
    ...
    function itemPembelian(urutan, key, value = null) {
        ...
        // Tambahkan select box untuk memilih klasifikasi pada keterangan
        item_pembelian += '<td><div class="form-group"><select class="form-control" style="font-size:14px" id="keterangan-' + urutan + '" name="keterangan[]">';
        @foreach ($klasifikasis as $klasifikasi)
        item_pembelian += '<option value="{{ $klasifikasi->nama }}"' + (keterangan === '{{ $klasifikasi->nama }}' ? ' selected' : '') + '>{{ $klasifikasi->nama }}</option>';
        @endforeach
        item_pembelian += '</select></div></td>';
