...
<div class="container">
    ...
    <form action="{{ route('inquery_penjualanprodukslawi.update', $penjualan->id) }}" method="POST">
                            ...
                            <tbody>
                                @foreach ($produks as $item)
                                    @php
                                        ...
                                        $stok_tokoslawi = $item->stok_tokoslawi ? $item->stok_tokoslawi->jumlah : 0; // Jika stok ada, tampilkan, jika tidak tampilkan 0
                                    @endphp
                                    <tr class="pilih-btn"
                                        data-id="{{ $item->id }}"
                                        ...
                                        <td class="text-center">
                                            {{ $stok_tokoslawi }} <!-- Tampilkan stok produk -->
                                        </td>
    ...
    <script>
        function getData1() {
            ...
            if (metodeId && document.querySelector('#nama_metode option:checked').text === 'Tunai') {
                ...
                $.ajax({
                    url: "{{ url('toko_slawi/metodebayarslawi/metode') }}" + "/" + metodeId,
                    type: "GET",
                    dataType: "json",
                    success: function(response) {
