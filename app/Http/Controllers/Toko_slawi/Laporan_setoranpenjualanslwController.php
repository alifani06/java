...
class Laporan_setoranpenjualanslwController extends Controller
{
    ...
    public function index(Request $request)
{
    ...
    $penjualan_kotor = Penjualanproduk::select(Penjualanproduk::raw(
    ...
    if ($kasir) {
        ...
        $penjualan_kotor->where('toko_id', 3);
    }
    ...
    $kasirs = Penjualanproduk::select('kasir')->where('toko_id', 3)->distinct()->get();

    return view('toko_slawi.laporan_setoranpenjualan.index', compact(
...
//     // Fungsi untuk membersihkan format "Rp" dan titik
...
//         ->get()
//         ->sum(function($item) use ($cleanCurrency) {
//             return $cleanCurrency($item->sub_totalasli);
//         });
...
//             $q->where('toko_id', 3);
//         }
//     })->get()->sum(function ($detail) use ($cleanCurrency) {
//         $harga = $cleanCurrency($detail->harga);
//         $jumlah = $detail->jumlah;
//         $diskon = $detail->diskon / 100;
...
//     $metodePembayaran = function($metode_id, $tanggal_penjualan = null, $tanggal_akhir = null) use ($kasir, $cleanCurrency) {
//         // Query untuk penjualan produk
//         $queryPenjualan = Penjualanproduk::where('metode_id', $metode_id);
...
//         $totalPenjualan = $queryPenjualan
//             ->get()
//             ->sum(function ($item) use ($metode_id, $cleanCurrency) {
//                 if ($metode_id == 1) {
//                     return $cleanCurrency($item->sub_totalasli) - $cleanCurrency($item->nominal_diskon);
//                 }
//                 return $cleanCurrency($item->sub_total);
//             });

//         // Query untuk pemesanan produk, namun menggunakan dp_pemesanan dari tabel dppemesanan
//         $totalPemesanan = Dppemesanan::whereHas('pemesananproduk', function ($q) use ($metode_id, $kasir, $tanggal_penjualan, $tanggal_akhir) {
//             $q->where('metode_id', $metode_id);
...
// }
    ...
    public function printReportsetoranslw(Request $request)
    {
        ...
        $pdf = FacadePdf::loadView('toko_slawi.laporan_setoranpenjualan.print', compact(
            'inquery',
            'kasir',
            'penjualan_kotor',
            'penjualan_bersih',
