...
class Laporan_historicilacapController extends Controller
{
    ...
    public function index(Request $request)
{
    ...
    // Kirim data ke view
    return view('toko_cilacap.laporan_historicilacap.index', compact('stokBarangJadi', 'tokos', 'klasifikasis', 'produks'));
}


public function barangMasukpesanancilacap(Request $request)
{
    $status = $request->status;
    $tanggal_pengiriman = $request->tanggal_pengiriman;
    $tanggal_akhir = $request->tanggal_akhir;
    $toko_id = 6;  // Tetapkan toko_id langsung menjadi 1
    $klasifikasi_id = $request->klasifikasi_id;
    ...
    // Kirim data ke view
    return view('toko_cilacap.laporan_historicilacap.barangmasukpesanan', compact('stokBarangJadi', 'tokos', 'klasifikasis', 'produks'));
}

    public function barangMasuksemuacilacap(Request $request)
{
    $status = $request->status;
    $tanggal_pengiriman = $request->tanggal_pengiriman;
    $tanggal_akhir = $request->tanggal_akhir;
    ...
    // Toko_id 1
    $query1->where('pengiriman_barangjadis.toko_id', 6);
    $query2->where('pengiriman_barangjadipesanans.toko_id', 6);
    ...
    // Kirim data ke view
    return view('toko_cilacap.laporan_historicilacap.barangmasuksemua', compact('stokBarangJadi', 'tokos', 'klasifikasis', 'produks'));
}

public function barangKeluarcilacap(Request $request)
{
    $status = $request->status;
    ...
    if (!$status && !$tanggal_penjualan && !$tanggal_akhir && !$klasifikasi_id && !$produk_id) {
        ...
        return view('toko_cilacap.laporan_historicilacap.barangkeluar', [
            'finalResults' => [],
            'tokos' => $tokos,
            'produks' => $produks,
    ...
    $inquery = Penjualanproduk::with('detailPenjualanProduk.produk')
        ->where('toko_id', 6) // Membatasi data hanya untuk toko_id = 1
...
public function barangKeluarRincicilacap(Request $request)
{
    $status = $request->status;
    $tanggal_penjualan = $request->tanggal_penjualan;
    $tanggal_akhir = $request->tanggal_akhir;
    ...
    if (!$status && !$tanggal_penjualan && !$tanggal_akhir && !$klasifikasi_id && !$produk_id) {
        // Jika tidak ada filter yang dipilih, kirim data kosong ke view
        return view('toko_cilacap.laporan_historicilacap.barangkeluarrinci', [
            'finalResults' => [],
            'tokos' => $tokos,
            'produks' => $produks,
            'klasifikasis' => $klasifikasis,
    ...
    public function barangReturcilacap(Request $request)
    {
        $status = $request->status;
        ...
        return view('toko_cilacap.laporan_historicilacap.barangretur', compact('stokBarangJadi', 'tokos', 'klasifikasis', 'totalJumlah', 'grandTotal'));
    }
    
    
    public function printLaporanBmcilacap(Request $request)
    {
        ...
        // Nama toko default
        $branchName = 'Toko Cilacap'; // Mengganti dengan nama toko yang sesuai
        ...
        $pdf = FacadePdf::loadView('toko_cilacap.laporan_historicilacap.printbm', [
    ...
    public function printLaporanBmpesanancilacap(Request $request)
    {
        // Ambil parameter filter dari request
        $status = $request->status;
        $tanggal_pengiriman = $request->tanggal_pengiriman;
        ...
        $query = Pengiriman_barangjadipesanan::join('produks', 'pengiriman_barangjadipesanans.produk_id', '=', 'produks.id')
            ...
            ->with('produk.klasifikasi')
            ->where('toko_id', 6); // Toko ID statis menjadi 3
        ...
        // Memuat konten HTML dari view
        $pdf = FacadePdf::loadView('toko_cilacap.laporan_historicilacap.printbmpesanan', [
            'groupedData' => $groupedData,
            'startDate' => $formattedStartDate,
            'endDate' => $formattedEndDate,
            'branchName' => $branchName,
    ...
    public function printLaporanBmsemuacilacap(Request $request)
    {
        // Ambil parameter filter dari request
        $status = $request->status;
        $tanggal_pengiriman = $request->tanggal_pengiriman;
        ...
        // Memuat konten HTML dari view
        $pdf = FacadePdf::loadView('toko_cilacap.laporan_historicilacap.printbmsemua', [
            'groupedData' => $groupedData,
            'startDate' => $formattedStartDate,
            'endDate' => $formattedEndDate,
            'branchName' => $branchName,
    ...
    public function printLaporanBKcilacap(Request $request)
{
    // Ambil parameter filter dari request
    $status = $request->status;
    $tanggal_penjualan = $request->tanggal_penjualan;
    ...
    $pdf = FacadePdf::loadView('toko_cilacap.laporan_historicilacap.printbk', [
        'finalResults' => $finalResults,
        'startDate' => $startDate,
        'endDate' => $endDate,
        'branchName' => $branchName,
    ...
    public function printLaporanBKrincicilacap(Request $request)
{
    // Ambil parameter filter dari request
    $status = $request->status;
    $tanggal_penjualan = $request->tanggal_penjualan;
    ...
    // Menggunakan Barryvdh\DomPDF\Facade\Pdf untuk memuat dan menghasilkan PDF
    $pdf = FacadePdf::loadView('toko_cilacap.laporan_historicilacap.printbkrinci', [
        'finalResults' => $finalResults,
        'startDate' => $startDate,
        'endDate' => $endDate,
        'branchName' => $branchName,
    ...
    public function printLaporanBRcilacap(Request $request)
{
    // Ambil parameter filter dari request
    $status = $request->status;
    $tanggal_retur = $request->tanggal_retur;
    ...
    $pdf = FacadePdf::loadView('toko_cilacap.laporan_historicilacap.printbr', [
...
public function barangOpercilacap(Request $request)
{
    $status = $request->status;
    ...
    return view('toko_cilacap.laporan_historicilacap.barangoper', compact('stokBarangJadi', 'tokos', 'klasifikasis', 'totalJumlah', 'grandTotal'));
}

public function barangOperancilacapMasuk(Request $request)
{
    $status = $request->status;
    $tanggal_input = $request->tanggal_input;
    $tanggal_akhir = $request->tanggal_akhir;
    ...
    $query = Pemindahan_tokocilacapmasuk::with(['produk.klasifikasi', 'toko']);
    ...
    return view('toko_cilacap.laporan_historicilacap.barangopermasuk', compact('stokBarangJadi',  'klasifikasis', 'totalJumlah', 'grandTotal'));
}

public function printLaporanBOcilacap(Request $request)
{
    // Ambil parameter filter dari request
    $status = $request->status;
    $tanggal_input = $request->tanggal_input;
    ...
    // Query dasar untuk mengambil data Retur_barangjadi
    $query = Pemindahan_tokocilacap::with('produk.klasifikasi')
        ->join('produks', 'produks.id', '=', 'pemindahan_tokocilacaps.produk_id') // Join with 'produks'
        ->orderBy('produks.kode_lama', 'asc') // Order by 'kode_lama'
        ->orderBy('pemindahan_tokocilacaps.tanggal_input', 'desc'); // Then order by 'tanggal_input'

    // Filter berdasarkan status
    if ($status) {
        $query->where('pemindahan_tokocilacaps.status', $status);
    }

    // Filter berdasarkan toko_id yang sudah ditetapkan menjadi 1
    $query->where('pemindahan_tokocilacaps.toko_id', $toko_id);
    ...
    if ($tanggal_input && $tanggal_akhir) {
        $tanggal_input = Carbon::parse($tanggal_input)->startOfDay();
        $tanggal_akhir = Carbon::parse($tanggal_akhir)->endOfDay();
        $query->whereBetween('pemindahan_tokocilacaps.tanggal_input', [$tanggal_input, $tanggal_akhir]);
    } elseif ($tanggal_input) {
        $tanggal_input = Carbon::parse($tanggal_input)->startOfDay();
        $query->where('pemindahan_tokocilacaps.tanggal_input', '>=', $tanggal_input);
    } elseif ($tanggal_akhir) {
        $tanggal_akhir = Carbon::parse($tanggal_akhir)->endOfDay();
        $query->where('pemindahan_tokocilacaps.tanggal_input', '<=', $tanggal_akhir);
    } else {
        // Jika tidak ada filter tanggal, tampilkan data untuk hari ini
        $query->whereDate('pemindahan_tokocilacaps.tanggal_input', Carbon::today());
    }

    // Eksekusi query dan dapatkan hasilnya
    $stokBarangJadi = $query->select('pemindahan_tokocilacaps.*') // Select only 'retur_barangjadi' fields
        ->get()
    ...
    // Menggunakan FacadePdf untuk menghasilkan PDF
    $pdf = FacadePdf::loadView('toko_cilacap.laporan_historicilacap.printbo', [
        'stokBarangJadi' => $stokBarangJadi,
        'startDate' => $formattedStartDate,
        'endDate' => $formattedEndDate,
        'branchName' => $branchName,
...
public function printLaporanBOcilacapMasuk(Request $request)
{
    // Ambil parameter filter dari request
    $status = $request->status;
    $tanggal_input = $request->tanggal_input;
    ...
    // Query dasar untuk mengambil data Retur_barangjadi
    $query = Pemindahan_tokocilacapmasuk::with('produk.klasifikasi')
        ->join('produks', 'produks.id', '=', 'pemindahan_tokocilacapmasuks.produk_id') // Join with 'produks'
        ->orderBy('produks.kode_lama', 'asc') // Order by 'kode_lama'
        ->orderBy('pemindahan_tokocilacapmasuks.tanggal_input', 'desc'); // Then order by 'tanggal_input'

    // Filter berdasarkan status
    if ($status) {
        $query->where('pemindahan_tokocilacapmasuks.status', $status);
    }
    ...
    if ($tanggal_input && $tanggal_akhir) {
        $tanggal_input = Carbon::parse($tanggal_input)->startOfDay();
        $tanggal_akhir = Carbon::parse($tanggal_akhir)->endOfDay();
        $query->whereBetween('pemindahan_tokocilacapmasuks.tanggal_input', [$tanggal_input, $tanggal_akhir]);
    } elseif ($tanggal_input) {
        $tanggal_input = Carbon::parse($tanggal_input)->startOfDay();
        $query->where('pemindahan_tokocilacapmasuks.tanggal_input', '>=', $tanggal_input);
    } elseif ($tanggal_akhir) {
        $tanggal_akhir = Carbon::parse($tanggal_akhir)->endOfDay();
        $query->where('pemindahan_tokocilacapmasuks.tanggal_input', '<=', $tanggal_akhir);
    } else {
        // Jika tidak ada filter tanggal, tampilkan data untuk hari ini
        $query->whereDate('pemindahan_tokocilacapmasuks.tanggal_input', Carbon::today());
    }

    // Eksekusi query dan dapatkan hasilnya
    $stokBarangJadi = $query->select('pemindahan_tokocilacapmasuks.*') // Select only 'retur_barangjadi' fields
        ->get()
        ->groupBy('kode_pemindahan');
    ...
    // Menggunakan FacadePdf untuk menghasilkan PDF
    $pdf = FacadePdf::loadView('toko_cilacap.laporan_historicilacap.printbomasuk', [
        'stokBarangJadi' => $stokBarangJadi,
        'startDate' => $formattedStartDate,
        'endDate' => $formattedEndDate,
        'totalJumlah' => $totalJumlah,
