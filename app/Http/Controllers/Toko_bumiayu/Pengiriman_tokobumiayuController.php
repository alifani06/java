...
class Pengiriman_tokobumiayuController extends Controller{
    ...
    // }
      ...
      {
          // Ambil data stok_tokobanjaran berdasarkan ID
          $stok = Pengirimanpemesanan_tokobumiayu::where('id', $id)->first();
          ...
          // Tambahkan jumlah ke stok_tokobanjarans
          $stokToko = Stokpesanan_tokobumiayu::where('produk_id', $pengirimanItem->produk_id)->first();
          if ($stokToko) {
              $stokToko->jumlah += $pengirimanItem->jumlah;
              $stokToko->save();
          }
