...
class Laporan_historibumiayuController extends Controller
{
    ...
    public function index(Request $request)
{
    ...
    $toko_id = 5;  // Tetapkan toko_id langsung menjadi 1
    ...
    // Kirim data ke view
    return view('toko_bumiayu.laporan_historibumiayu.index', compact('stokBarangJadi', 'tokos', 'klasifikasis', 'produks'));
}


public function barangMasukpesananbumiayu(Request $request)
{
    ...
    // Kirim data ke view
    return view('toko_bumiayu.laporan_historibumiayu.barangmasukpesanan', compact('stokBarangJadi', 'tokos', 'klasifikasis', 'produks'));
}

    public function barangMasuksemuabumiayu(Request $request)
{
    $status = $request->status;
    $tanggal_pengiriman = $request->tanggal_pengiriman;
    $tanggal_akhir = $request->tanggal_akhir;
    ...
    // Kirim data ke view
    return view('toko_bumiayu.laporan_historibumiayu.barangmasuksemua', compact('stokBarangJadi', 'tokos', 'klasifikasis', 'produks'));
}

public function barangKeluarbumiayu(Request $request)
{
    $status = $request->status;
    ...
    if (!$status && !$tanggal_penjualan && !$tanggal_akhir && !$klasifikasi_id && !$produk_id) {
        ...
        return view('toko_bumiayu.laporan_historibumiayu.barangkeluar', [
            'finalResults' => [],
            'tokos' => $tokos,
            'produks' => $produks,
    ...
    $inquery = Penjualanproduk::with('detailPenjualanProduk.produk')
        ->where('toko_id', 5) // Membatasi data hanya untuk toko_id = 1
...
public function barangKeluarRincibumiayu(Request $request)
{
    $status = $request->status;
    $tanggal_penjualan = $request->tanggal_penjualan;
    $tanggal_akhir = $request->tanggal_akhir;
    ...
    if (!$status && !$tanggal_penjualan && !$tanggal_akhir && !$klasifikasi_id && !$produk_id) {
        // Jika tidak ada filter yang dipilih, kirim data kosong ke view
        return view('toko_bumiayu.laporan_historibumiayu.barangkeluarrinci', [
            'finalResults' => [],
            'tokos' => $tokos,
            'produks' => $produks,
            'klasifikasis' => $klasifikasis,
    ...
    public function barangReturbumiayu(Request $request)
    {
        $status = $request->status;
        ...
        return view('toko_bumiayu.laporan_historibumiayu.barangretur', compact('stokBarangJadi', 'tokos', 'klasifikasis', 'totalJumlah', 'grandTotal'));
    }
    
    
    public function printLaporanBmbumiayu(Request $request)
    {
        ...
        // Nama toko default
        $branchName = 'Toko Bumiayu'; // Mengganti dengan nama toko yang sesuai
        ...
        $pdf = FacadePdf::loadView('toko_bumiayu.laporan_historibumiayu.printbm', [
    ...
    public function printLaporanBmpesananbumiayu(Request $request)
    {
        // Ambil parameter filter dari request
        $status = $request->status;
        $tanggal_pengiriman = $request->tanggal_pengiriman;
        ...
        // Memuat konten HTML dari view
        $pdf = FacadePdf::loadView('toko_bumiayu.laporan_historibumiayu.printbmpesanan', [
            'groupedData' => $groupedData,
            'startDate' => $formattedStartDate,
            'endDate' => $formattedEndDate,
            'branchName' => $branchName,
    ...
    public function printLaporanBmsemuabumiayu(Request $request)
    {
        // Ambil parameter filter dari request
        $status = $request->status;
        $tanggal_pengiriman = $request->tanggal_pengiriman;
        ...
        // Memuat konten HTML dari view
        $pdf = FacadePdf::loadView('toko_bumiayu.laporan_historibumiayu.printbmsemua', [
            'groupedData' => $groupedData,
            'startDate' => $formattedStartDate,
            'endDate' => $formattedEndDate,
            'branchName' => $branchName,
    ...
    public function printLaporanBKbumiayu(Request $request)
{
    // Ambil parameter filter dari request
    $status = $request->status;
    $tanggal_penjualan = $request->tanggal_penjualan;
    ...
    $pdf = FacadePdf::loadView('toko_bumiayu.laporan_historibumiayu.printbk', [
        'finalResults' => $finalResults,
        'startDate' => $startDate,
        'endDate' => $endDate,
        'branchName' => $branchName,
    ...
    public function printLaporanBKrincibumiayu(Request $request)
{
    // Ambil parameter filter dari request
    $status = $request->status;
    $tanggal_penjualan = $request->tanggal_penjualan;
    ...
    // Menggunakan Barryvdh\DomPDF\Facade\Pdf untuk memuat dan menghasilkan PDF
    $pdf = FacadePdf::loadView('toko_bumiayu.laporan_historibumiayu.printbkrinci', [
        'finalResults' => $finalResults,
        'startDate' => $startDate,
        'endDate' => $endDate,
        'branchName' => $branchName,
    ...
    public function printLaporanBRbumiayu(Request $request)
{
    // Ambil parameter filter dari request
    $status = $request->status;
    $tanggal_retur = $request->tanggal_retur;
    ...
    $pdf = FacadePdf::loadView('toko_bumiayu.laporan_historibumiayu.printbr', [
...
public function barangOperbumiayu(Request $request)
{
    $status = $request->status;
    ...
    return view('toko_bumiayu.laporan_historibumiayu.barangoper', compact('stokBarangJadi', 'tokos', 'klasifikasis', 'totalJumlah', 'grandTotal'));
}

public function barangOperanbumiayuMasuk(Request $request)
{
    $status = $request->status;
    $tanggal_input = $request->tanggal_input;
    $tanggal_akhir = $request->tanggal_akhir;
    ...
    $query = Pemindahan_tokobumiayumasuk::with('produk.klasifikasi');
    ...
    return view('toko_bumiayu.laporan_historibumiayu.barangopermasuk', compact('stokBarangJadi',  'klasifikasis', 'totalJumlah', 'grandTotal'));
}

public function printLaporanBObumiayu(Request $request)
{
    // Ambil parameter filter dari request
    $status = $request->status;
    $tanggal_input = $request->tanggal_input;
    ...
    // Query dasar untuk mengambil data Retur_barangjadi
    $query = Pemindahan_tokobumiayu::with('produk.klasifikasi')
        ->join('produks', 'produks.id', '=', 'pemindahan_tokobumiayus.produk_id') // Join with 'produks'
        ->orderBy('produks.kode_lama', 'asc') // Order by 'kode_lama'
        ->orderBy('pemindahan_tokobumiayus.tanggal_input', 'desc'); // Then order by 'tanggal_input'

    // Filter berdasarkan status
    if ($status) {
        $query->where('pemindahan_tokobumiayus.status', $status);
    }

    // Filter berdasarkan toko_id yang sudah ditetapkan menjadi 1
    $query->where('pemindahan_tokobumiayus.toko_id', $toko_id);
    ...
    if ($tanggal_input && $tanggal_akhir) {
        $tanggal_input = Carbon::parse($tanggal_input)->startOfDay();
        $tanggal_akhir = Carbon::parse($tanggal_akhir)->endOfDay();
        $query->whereBetween('pemindahan_tokobumiayus.tanggal_input', [$tanggal_input, $tanggal_akhir]);
    } elseif ($tanggal_input) {
        $tanggal_input = Carbon::parse($tanggal_input)->startOfDay();
        $query->where('pemindahan_tokobumiayus.tanggal_input', '>=', $tanggal_input);
    } elseif ($tanggal_akhir) {
        $tanggal_akhir = Carbon::parse($tanggal_akhir)->endOfDay();
        $query->where('pemindahan_tokobumiayus.tanggal_input', '<=', $tanggal_akhir);
    } else {
        // Jika tidak ada filter tanggal, tampilkan data untuk hari ini
        $query->whereDate('pemindahan_tokobumiayus.tanggal_input', Carbon::today());
    }

    // Eksekusi query dan dapatkan hasilnya
    $stokBarangJadi = $query->select('pemindahan_tokobumiayus.*') // Select only 'retur_barangjadi' fields
        ->get()
    ...
    // Menggunakan FacadePdf untuk menghasilkan PDF
    $pdf = FacadePdf::loadView('toko_bumiayu.laporan_historibumiayu.printbo', [
        'stokBarangJadi' => $stokBarangJadi,
        'startDate' => $formattedStartDate,
        'endDate' => $formattedEndDate,
        'branchName' => $branchName,
...
public function printLaporanBObumiayuMasuk(Request $request)
{
    // Ambil parameter filter dari request
    $status = $request->status;
    $tanggal_input = $request->tanggal_input;
    ...
    // Query dasar untuk mengambil data Retur_barangjadi
    $query = Pemindahan_tokobumiayumasuk::with('produk.klasifikasi')
        ->join('produks', 'produks.id', '=', 'pemindahan_tokobumiayumasuks.produk_id') // Join with 'produks'
        ->orderBy('produks.kode_lama', 'asc') // Order by 'kode_lama'
        ->orderBy('pemindahan_tokobumiayumasuks.tanggal_input', 'desc'); // Then order by 'tanggal_input'

    // Filter berdasarkan status
    if ($status) {
        $query->where('pemindahan_tokobumiayumasuks.status', $status);
    }
    ...
    if ($tanggal_input && $tanggal_akhir) {
        $tanggal_input = Carbon::parse($tanggal_input)->startOfDay();
        $tanggal_akhir = Carbon::parse($tanggal_akhir)->endOfDay();
        $query->whereBetween('pemindahan_tokobumiayumasuks.tanggal_input', [$tanggal_input, $tanggal_akhir]);
    } elseif ($tanggal_input) {
        $tanggal_input = Carbon::parse($tanggal_input)->startOfDay();
        $query->where('pemindahan_tokobumiayumasuks.tanggal_input', '>=', $tanggal_input);
    } elseif ($tanggal_akhir) {
        $tanggal_akhir = Carbon::parse($tanggal_akhir)->endOfDay();
        $query->where('pemindahan_tokobumiayumasuks.tanggal_input', '<=', $tanggal_akhir);
    } else {
        // Jika tidak ada filter tanggal, tampilkan data untuk hari ini
        $query->whereDate('pemindahan_tokobumiayumasuks.tanggal_input', Carbon::today());
    }

    // Eksekusi query dan dapatkan hasilnya
    $stokBarangJadi = $query->select('pemindahan_tokobumiayumasuks.*') // Select only 'retur_barangjadi' fields
        ->get()
        ->groupBy('kode_pemindahan');
    ...
    // Menggunakan FacadePdf untuk menghasilkan PDF
    $pdf = FacadePdf::loadView('toko_bumiayu.laporan_historibumiayu.printbomasuk', [
        'stokBarangJadi' => $stokBarangJadi,
        'startDate' => $formattedStartDate,
        'endDate' => $formattedEndDate,
        'totalJumlah' => $totalJumlah,
